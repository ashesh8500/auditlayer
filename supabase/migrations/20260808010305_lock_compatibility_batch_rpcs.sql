-- The rolling idempotency migration removed the permanent unique key so callers
-- can intentionally repeat an operation after ten minutes. Serialize the legacy
-- compatibility RPCs explicitly so they retain race-safe behavior without that
-- constraint.
...
declare
  ...
  v_audit_id uuid;
begin
  -- One lock per tenant serializes the retry lookup and insert.
  perform 1
  ...
  -- Hold the ownership claim through the transaction.
  perform 1
  ...
  foreach v_audit_id in array p_audit_ids loop
    ...
    for share;
    if not found then
      raise exception 'audit_not_owned';
    end if;
  end loop;
