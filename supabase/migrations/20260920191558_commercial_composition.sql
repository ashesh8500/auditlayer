-- New opt-in enrollment; preserve legacy plan and gifts, share ordered authority.
begin;
alter table public.profiles add column commercial_plan text check(commercial_plan in ('brand','studio'));
alter table public.provider_event_receipts drop constraint provider_event_receipts_plan_check;
alter table public.provider_event_receipts add constraint provider_event_receipts_plan_check check(plan in ('free','starter','pro','enterprise','brand','studio'));
-- Additive repair for databases that applied the original unshipped stage one.
-- Fresh rollout already has this identical authority BEFORE backfill/stage two.
...
create table public.commercial_checkouts (
 id uuid primary key default gen_random_uuid(), owner_id uuid not null references public.profiles(id),
 plan text not null check(plan in ('brand','studio')), session_id text unique,
 state text not null default 'pending' check(state in ('pending','completed','expired')),
 created_at timestamptz not null default now()
);
create unique index commercial_one_pending_checkout on public.commercial_checkouts(owner_id) where state='pending';
alter table public.commercial_checkouts enable row level security;
revoke all on public.commercial_checkouts from public,anon,authenticated,service_role;
grant select on public.commercial_checkouts to authenticated,service_role;
create policy owner_read on public.commercial_checkouts for select to authenticated using(owner_id=(select auth.uid()));
create function public.commercial_checkout_reserve(p jsonb) returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
...
create function public.commercial_checkout_bind(p jsonb) returns uuid language plpgsql security definer set search_path=pg_catalog,public as $$
declare c public.commercial_checkouts;
begin
 perform 1 from public.profiles where id=(p->>'owner_id')::uuid for update;
 select * into c from public.commercial_checkouts where id=(p->>'id')::uuid and owner_id=(p->>'owner_id')::uuid for update;
 if not found or coalesce(p->>'session_id','')='' or (c.session_id is not null and c.session_id<>p->>'session_id') then raise exception 'checkout_binding_mismatch'; end if;
 update public.commercial_checkouts set session_id=p->>'session_id' where id=c.id;
 return c.id;
end $$;
create function public.commercial_subscription_apply(p jsonb) returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
...
-- Only a signed provider expiry can free ambiguous pending capacity. A browser
-- cancel/back click never proves the hosted checkout cannot still charge.
create function public.commercial_checkout_expire(p jsonb) returns uuid language plpgsql security definer set search_path=pg_catalog,public as $$
declare c public.commercial_checkouts;
begin
 perform 1 from public.profiles where id=(p->>'owner_id')::uuid for update;
 select * into c from public.commercial_checkouts where id=(p->>'id')::uuid and owner_id=(p->>'owner_id')::uuid for update;
 if not found or coalesce(p->>'session_id','')='' or (c.session_id is not null and c.session_id<>p->>'session_id') then raise exception 'checkout_binding_mismatch'; end if;
 if c.state='pending' then update public.commercial_checkouts set state='expired',session_id=p->>'session_id' where id=c.id; end if;
 return c.id;
end $$;
revoke all on function public.commercial_checkout_reserve(jsonb),public.commercial_checkout_bind(jsonb),public.commercial_subscription_apply(jsonb),public.commercial_checkout_expire(jsonb) from public,anon,authenticated;
grant execute on function public.commercial_checkout_reserve(jsonb),public.commercial_checkout_bind(jsonb),public.commercial_subscription_apply(jsonb),public.commercial_checkout_expire(jsonb) to service_role;
commit;
