-- Persist the Graph API family explicitly instead of inferring it from token text.
-- Existing rows intentionally remain NULL and must reconnect before worker use.
-- Idempotent. The release gate owns production application.

alter table public.instagram_connections
  add column if not exists graph_api_family text;

alter table public.instagram_connections
  drop constraint if exists instagram_connections_graph_api_family_check;
alter table public.instagram_connections
  add constraint instagram_connections_graph_api_family_check
  check (
    graph_api_family is null
    or graph_api_family in ('instagram', 'facebook')
  );

comment on column public.instagram_connections.graph_api_family is
  'Explicit Graph API token family: instagram for direct Instagram Login or facebook for Facebook-backed tokens. NULL denotes a legacy connection that must reconnect; never infer or backfill from token text.';

-- Remove the previous overload so every writer must provide an explicit family.
