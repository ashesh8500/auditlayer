-- Keep the deployed eight-argument OAuth writer available during rollout/rollback.
-- Older clients cannot prove an Insights grant or explicit Graph family. Preserve
-- their connection for compatibility but require reauthorization in the new app.
-- Apply with the two preceding Instagram migrations in one transaction.
...
begin
  ...
  on conflict (user_id, ig_user_id) do update set
    ...
    graph_api_family = null,
    connection_status = 'reconnect_required',
    reconnect_required_at = now(),
    reconnect_reason = 'legacy_connection',
    is_active = true,
