...
agentic view**, renders self-contained HTML, uploads it to private Supabase
Storage, and records `tokens_in` / `tokens_out` / `cost_usd`
for billing.
...
GenerationPipeline ── emits audit_events ──▶ Supabase Realtime ──▶ frontend timeline
   ...
   ├─ Storage bucket "reports"  (private text/html)
   └─ audits row: status=ready, report_path, tokens_in/out, cost_usd
```
...
Runs a full generation against the local Hermes gateway and writes the HTML to
`AUDITLAYER_OUTPUT_DIR`, printing the event stream. If Hermes is
...
|---|---|---|
| `audits` | `handle, platform, goal, context, limitations, status=queued` | `status, report_path, tokens_in, tokens_out, cost_usd, model, milestone_label, admin_notes` |
...
Storage bucket (private): `reports` (self-contained text/html).
...
```bash
uv run pytest          # offline tests — mock generator, guardrails, calibration, scoring
