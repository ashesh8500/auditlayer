...
## Deploy sequence
worker: snapshot `/opt/auditlayer/worker` to `/opt/auditlayer/worker.prev`, then `sudo rsync -a --delete --exclude .venv --exclude __pycache__ --exclude '*.pyc' --exclude var/ --exclude .env <verified-release-root>/worker/ /opt/auditlayer/worker/ && sudo chown -R auditlayer:auditlayer /opt/auditlayer/worker /opt/auditlayer/.hermes /opt/alm/hermes/accounts && sudo -u auditlayer bash -lc 'cd /opt/auditlayer/worker && /usr/local/bin/uv sync --frozen --no-dev --extra embedded' && sudo systemctl restart auditlayer-worker@{1,2}`. The explicit embedded-extra sync is mandatory: a clean worker environment otherwise lacks the Hermes runtime dependencies even when the previous VM venv happened to contain them.
...
CRITICAL: ALWAYS chown /opt/alm/hermes/accounts too. The auditlayer system user owns the worker process but /opt/alm/hermes/accounts defaults to the deployer's user. Without this, every new audit fails at account home setup.
