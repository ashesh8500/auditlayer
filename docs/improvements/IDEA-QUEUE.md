...
| in_flight | P2 · C5 · F5 | Produce a deterministic audit-to-audit change classifier | Fixture-backed versions classify every delta as evidence, brief/lens, methodology, or correction; unexplained deltas become UNKNOWN with a correction tip; backward-compatible serialization passes | engineering |
...
| in_flight | P12 · C15 · F9 · D7 | Build an authentication and capability preflight matrix | Deterministic checks cover supported login methods, OAuth/env presence without secret disclosure, callback/deletion/support requirements, worker commands, model features, migrations, and failure/recovery states; missing capability blocks before mutation | reliability |
