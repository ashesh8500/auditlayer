...
**Commercial research is conditional on the pinned quote's `research_policy`.** With a null policy, commercial execution does not invoke the legacy paid Hermes managed-search tools: it uses the deadline-bounded public search-index path, connected first-party metrics, cached evidence and the pinned supplied brief. That legacy managed-tool path has no enforceable tariff/receipt contract; its historical `$0.12` estimate is not a hard limit. Null-policy inference admission subtracts the quoted research allowance, and commercial telemetry does not invent research spend.

With a non-null validated policy (`20260920214155_openrouter_research_contract.sql`), `commercial.pinned_settings` carries the pin into the ordinary generator and `InProcessHermesClient.collect_research` dispatches fixed OpenRouter/Exa research **before** the commercial public-index branch. This is one Exa fast search, three results maximum, finite context/output/fee reservation, durable pre-dispatch and terminal receipts, zero SDK retry and no automatic alternate search/provider. A search key alone does not enable it. Legacy noncommercial managed research remains unchanged.

Qualify the branch actually pinned and deployed; neither branch proves the other. The 2026-09-23 isolated policy-bearing run made one actual paid research call and rejected a fictional homepage citation; it generated no report. Its authorized local 1,398,976-token / $0.206253 reservation does not change the production 120k cap or prove production SQL settlement. See `real-provider-run-20260923.md` and `public-acquisition-repair-20260923.md`.
...
4. Record a real qualification row in the otherwise-empty `commercial_runtime_catalog`, with verified rates, bounds, expiry and evidence-bearing output using the exact pinned research-policy branch (or policy-null public-index/connected path). Test-only catalog rows exist only in disposable containers.
