...
import type { ConnectionsDTO } from "@/lib/resources/dto";
...
export function ConnectionsLibrary({ params }: { params: { page?: string; connection_id?: string; return_to?: string; instagram_connected?: string; instagram_error?: string; disconnected?: string } }) {
  const page = /^\d{1,6}$/.test(params.page ?? "") ? Math.max(1, Number(params.page)) : 1;
  const returnTo = safeInstagramReturnPath(params.return_to);
  const query = useWorkspaceQuery<ConnectionsDTO>("connections", new URLSearchParams({ page: String(page), ...(params.connection_id ? { connection_id: params.connection_id } : {}) }).toString());
  if (!query.data) return <ResourceStatus query={query} label="Connections" />;
  const { connections, target, count } = query.data;
  const pageHref = (next: number) => `/settings/connections?${new URLSearchParams({ page: String(next), ...(params.return_to ? { return_to: returnTo } : {}) })}`;

  return <main data-testid="connections-content" className="alm-shell py-8 sm:py-12 animate-page-in">
    ...
    <ResourceStatus query={query} label="Connections" />
