        ...
        <section className="mt-14 w-full">
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2 w-full">
            ...
            <div className="relative overflow-hidden rounded-[var(--radius-lg)] border border-border bg-white shadow-[var(--shadow-md)]">
              <div className="flex items-center justify-between border-b border-border bg-[#f5f5f4] px-5 py-3">
                <span className="font-mono text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">
                  Standard · 15 sections · $30/mo
                </span>
                <span className="rounded-full bg-[color:var(--accent-muted)] px-2.5 py-0.5 font-mono text-[0.65rem] font-semibold uppercase text-[color:var(--accent)]">
                  Starter+
                </span>
              </div>
              <div className="max-h-[620px] overflow-y-auto px-5 py-4 sm:px-6 sm:py-5">
                <p className="text-[0.65rem] font-bold uppercase tracking-[0.1em] text-[color:var(--accent)]">Instagram Account Audit</p>
                <h3 className="mt-0.5 text-lg font-bold tracking-[-0.02em]">@glowstate</h3>
                <p className="mt-0.5 text-xs text-muted-foreground leading-relaxed">
                  Premium adaptogenic supplements — functional mushrooms, nootropics, and plant-based wellness.
                </p>

                {/* 1. Executive Summary */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">1</span>
                  Executive Summary
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  @glowstate holds <strong className="text-foreground">24.8K followers</strong> with a clean visual identity and strong product differentiation. Engagement sits at <strong className="text-foreground">1.42%</strong> — half the 3.5% niche benchmark. Reels account for only 22% of output despite being the highest-reach format. The core issue: the account treats Instagram as a catalog, not a publication. Shifting to 60% Reels, adding story highlight architecture, and building a content-to-commerce pipeline can reach 40K within 90 days.
                </p>

                {/* 2. Key Metrics */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">2</span>
                  Key Metrics
                </h4>
                <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {[
                    ...
                    <div key={m.label} className="rounded-[var(--radius-sm)] border border-border p-2.5 text-center">
                      <div className={`font-mono text-base font-semibold leading-none ${m.tone || ""}`}>{m.value}</div>
                      <div className="mt-0.5 text-[0.55rem] font-medium uppercase tracking-[0.05em] text-muted-foreground">{m.label}</div>
                    </div>
                  ))}
                </div>
                ...
                {/* 3. Strengths */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">3</span>
                  Strengths
                </h4>
                <div className="mt-2 space-y-2">
                  {[
                ...
                {/* 4. Weaknesses */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">4</span>
                  Weaknesses
                </h4>
                <div className="mt-2 space-y-2">
                  {[
                    "Engagement rate at 1.42% vs 3.5% niche benchmark — likes are shallow, saves and shares are near zero.",
                    "No story highlight architecture — zero highlights despite 4–5 posts/week. Missed trust-building real estate that every profile visitor sees.",
                    "Content-to-commerce gap — every post is brand-building with no purchase path or conversion mechanism.",
                  ].map((s) => (
                    <div key={s} className="rounded-[var(--radius-sm)] border-l-2 border-l-[color:var(--red)] bg-[#fcfcfb] p-3">
                ...
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">5</span>
                  Root Cause Analysis
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  The account was built as a product showcase, not a media publication. Every post answers &ldquo;what do we sell&rdquo; instead of &ldquo;what problem do we solve.&rdquo; This catalog mindset produces shallow engagement because there&apos;s nothing to save, share, or return to. The format mix reinforces the problem — photo-heavy (78%) when Reels and carousels drive 3x more discovery. The root cause is editorial philosophy, not execution quality.
                </p>

                {/* 6. Peer Comparison */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">6</span>
                  Peer Comparison
                </h4>
                <div className="mt-2 overflow-x-auto rounded-[var(--radius-sm)] border border-border">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border bg-[#f5f5f4] text-left">
                        <th className="px-3 py-2 font-semibold">Account</th>
                        <th className="px-3 py-2 font-semibold">Followers</th>
                        <th className="px-3 py-2 font-semibold">ER</th>
                        <th className="px-3 py-2 font-semibold">Format Mix</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        ["@glowstate", "24.8K", "1.42%", "78% photos, 22% Reels"],
                        ["@fourigmatic", "31K", "3.1%", "60% Reels, 25% carousels"],
                        ["@mudwtr", "112K", "2.8%", "Lifestyle mix, story-first"],
                      ].map((row, i) => (
                        <tr key={i} className={`border-b border-border last:border-0 ${i === 0 ? "bg-[color:var(--accent-muted)]/20 font-semibold" : ""}`}>
                ...
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">7</span>
                  Content Format Analysis
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  Reels account for 22% of output but drive 58% of reach. Carousels perform 3x better than single images on save rate — the highest-intent engagement signal. Stories are completely absent: zero highlights means the profile has no persistent trust architecture for first-time visitors. Single-image posts dominate at 68% despite being the lowest-performing format across every metric.
                </p>

                {/* 8. Engagement Growth Strategy */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">8</span>
                  Engagement Growth Strategy
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  Shift to 60% Reels with hook-first editing — open with a pattern interrupt, not a product shot. Build 5 story highlights: Product Science, Reviews, Founder, Behind the Scenes, FAQ. Reduce reply latency to under 2 hours — current average is 8+ hours. Implement weekly save-bait carousels (educational threads, comparison guides, myth-busting). Launch one collaborative Reel per month with a complementary brand.
                </p>

                {/* 9. Quick Wins */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">9</span>
                  Quick Wins — This Week
                </h4>
                <div className="mt-2 space-y-2">
                  {[
                    "Post one Reel using trending audio in the adaptogen niche — open with a bold claim, not a product shot.",
                    "Create &lsquo;Product Science&rsquo; story highlight with 8 slides explaining your extraction process.",
                    "Reply to every comment from the last 5 posts within 2 hours — use questions to extend threads.",
                  ].map((w, i) => (
                    <div key={i} className="flex gap-3 text-xs text-muted-foreground">
                      <span className="flex-shrink-0 grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">{i + 1}</span>
                      {w}
                    </div>
                  ))}
                </div>
                ...
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">10</span>
                  Success Benchmarks
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  <strong>30 days:</strong> Engagement rate above 2.2%, Reels at 40% of output, reply latency under 4 hours. <strong>60 days:</strong> First collaborative Reel published, story highlight architecture live, save rate above 1.5%. <strong>90 days:</strong> Engagement above 3%, 60% Reels cadence, 40K followers.
                </p>

                {/* 11. Audience Profile */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">11</span>
                  Audience Profile
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  Primary: Health-conscious 25–40 year olds seeking evidence-backed supplements. Secondary: Fitness enthusiasts and biohackers exploring functional mushrooms and nootropics. Tertiary: Wellness-curious beginners intimidated by clinical language — a growth opportunity for accessible educational content.
                </p>

                {/* 12. Road to 40K */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">12</span>
                  Road to 40K
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  <strong>Phase 1 (Days 1–30):</strong> Fix the format mix — 60% Reels, 25% carousels, 15% photos. Build story highlights. Reduce reply time. Target: 28K. <strong>Phase 2 (Days 31–60):</strong> Launch collaborative Reels, establish save-bait carousel cadence, begin weekly educational threads. Target: 33K. <strong>Phase 3 (Days 61–90):</strong> Optimize top-performing formats, scale what works, launch content-to-commerce bridge posts. Target: 40K.
                </p>

                {/* 13. Audit Cadence */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">13</span>
                  Audit Cadence
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  Re-audit every 30 days for the first 90 days to track format mix shifts and engagement trajectory. After 90 days, shift to quarterly re-audits. Event-triggered re-audit after any product launch, collaboration, or viral moment.
                </p>

                {/* 14. Footer */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">14</span>
                  Footer
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  This report is a strategic assessment based on publicly available data, platform benchmarks, and industry-standard analytics. Metrics are estimates drawn from indexed public content; exact platform analytics may differ. AuditLayerMedia is not affiliated with Instagram or any audited platform. Always verify independently before making business decisions.
                </p>

                {/* 15. Powered by AuditLayerMedia */}
                <div className="mt-6 rounded-md bg-[#1c1917] px-4 py-3 text-center">
                  <span className="text-sm font-bold text-white tracking-tight">ALM</span>
                  <span className="ml-2 text-xs text-white/70">Powered by AuditLayerMedia</span>
                  <p className="mt-1 text-[0.6rem] text-white/50">@auditlayermedia</p>
                </div>
              </div>
            ...
            <div className="relative overflow-hidden rounded-[var(--radius-lg)] border-2 border-[color:var(--accent)] bg-white shadow-[var(--shadow-lg)]">
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-10">
                <span className="rounded-full bg-[color:var(--accent)] px-4 py-1 text-[0.65rem] font-bold uppercase text-white tracking-wider">
                  Recommended
                </span>
              </div>
              <div className="flex items-center justify-between border-b border-border bg-[color:var(--accent-muted)] px-5 py-3">
                <span className="font-mono text-xs font-semibold uppercase tracking-[0.1em] text-[color:var(--accent)]">
                  Extended · 20 sections · $50/mo
                </span>
                <span className="rounded-full bg-[color:var(--accent)] px-2.5 py-0.5 font-mono text-[0.65rem] font-semibold uppercase text-white">
                  Pro
                </span>
              </div>
              <div className="max-h-[620px] overflow-y-auto px-5 py-4 sm:px-6 sm:py-5">
                ...
                {/* 1-3: Condensed summary — same as Standard for core sections */}
                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">1</span>
                  Executive Summary
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  @glowstate holds 24.8K followers with strong product differentiation but engagement at 1.42% — half the 3.5% niche benchmark. The account operates as a product catalog rather than a publication. Shifting to 60% Reels, building story architecture, and adding a content-to-commerce pipeline targets 40K within 90 days.
                </p>
                ...
                <div className="mt-2 space-y-2">
                  {[
                    { title: "Product Differentiation", body: "Dual-extract formulations and third-party lab testing are rare structural moats." },
                    { title: "Visual Identity", body: "Forest green + amber palette is consistent and intentional." },
                    { title: "Posting Consistency", body: "4–5 posts/week over 8+ months. Habit infrastructure is in place." },
                  ].map((s) => (
                    <div key={s.title} className="rounded-[var(--radius-sm)] border border-border bg-[#fcfcfb] p-3">
                      <p className="text-xs"><strong className="text-foreground">{s.title}.</strong> <span className="text-muted-foreground">{s.body}</span></p>
                    </div>
                ...
                <div className="mt-2 space-y-2">
                  {[
                    "Engagement at 1.42% vs 3.5% benchmark — likes are shallow, saves near zero.",
                    "No story highlight architecture — missed trust-building real estate.",
                    "Content-to-commerce gap — no purchase path or conversion mechanism.",
                  ].map((s) => (
                    <div key={s} className="rounded-[var(--radius-sm)] border-l-2 border-l-[color:var(--red)] bg-[#fcfcfb] p-3">
                      <p className="text-xs text-muted-foreground leading-relaxed">{s}</p>
                    </div>
                ...
                {/* Sections 5-15: Abbreviated for space */}
                <div className="mt-5 rounded-[var(--radius-sm)] border border-border/50 bg-[#fafaf9] p-3">
                  <p className="text-[0.65rem] font-semibold text-muted-foreground">
                    Sections 5–15 follow the Standard framework:
                  </p>
                  <p className="mt-1 text-[0.6rem] text-muted-foreground leading-relaxed">
                    Root Cause Analysis · Peer Comparison · Content Format Analysis · Engagement Growth Strategy · Quick Wins — This Week · Success Benchmarks · Audience Profile · Road to 40K · Audit Cadence · Content Pillars & Ideas · Footer
                  </p>
                </div>

                {/* ── Premium Sections 16-20 ── */}
                <div className="mt-5 rounded-[var(--radius)] border-2 border-[color:var(--accent)]/30 bg-[color:var(--accent-muted)]/50 p-4">
                  <p className="text-xs font-bold uppercase tracking-[0.1em] text-[color:var(--accent)]">✦ Premium Sections</p>
                  <p className="mt-0.5 text-[0.6rem] text-muted-foreground">Only in Extended. Five advanced sections for creators ready to dominate.</p>
                </div>

                <h4 className="mt-4 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">16</span>
                  Power of Posting Stories
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  A story isn&apos;t a repost of your feed content — it&apos;s a different medium with different psychology. Stories create intimacy through impermanence: behind-the-scenes glimpses, polls that invite participation, Q&A sessions that build parasocial trust, raw unfiltered moments that humanize the brand. Accounts that post 5+ stories per day see <strong className="text-foreground">2.3x higher profile visits</strong> than those posting 0–1. Build 5 themed story highlights that serve as a permanent trust portfolio: Product Science, Reviews & Testimonials, Founder&apos;s Philosophy, Behind the Scenes, and FAQ. Each highlight should have 6–10 slides. Update weekly.
                </p>

                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">17</span>
                  Your Thumbnails Are the Lens
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  Your thumbnail IS your first impression — the lens through which every potential follower decides whether to stop scrolling or keep moving. Use <strong className="text-foreground">face-in-context</strong> (a person in the frame creates instant relatability), <strong className="text-foreground">promise-driven text overlays</strong> (tell them what they&apos;ll learn before they click), and <strong className="text-foreground">consistent color grading</strong> across your grid (recognizable at a glance in the explore feed). Test every thumbnail: if it doesn&apos;t stop YOU at 2x scroll speed, it won&apos;t stop anyone. @glowstate&apos;s current thumbnails lack text overlays entirely — the account is invisible in the explore feed.
                </p>

                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">18</span>
                  Leave Genuine Comments
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  Start leaving real, specific comments under posts from creators you resonate with — not &ldquo;great post!&rdquo; but thoughtful replies that add value to the conversation. This is the <strong className="text-foreground">most underrated growth lever on Instagram</strong>. Genuine comments signal to the algorithm that you&apos;re an active community member, not a passive consumer. They also put your handle in front of that creator&apos;s audience — every comment is a micro-impression. Aim for 5 meaningful comments per day on accounts in the adaptogen/wellness niche. Your comment history is currently near zero — you&apos;re invisible to the community graph. This alone can drive 200–500 new profile visits per month.
                </p>

                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">19</span>
                  Your First 3 Seconds
                </h4>
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  The first 3 seconds of any video determine whether someone stays or scrolls. Open with a <strong className="text-foreground">pattern interrupt</strong> — a question that creates curiosity, a bold claim that demands verification, or a visual surprise that breaks the scroll rhythm. Never open with &ldquo;Hey guys&rdquo; or a logo animation. The hook must create enough curiosity to survive the thumb-release reflex — the physical moment a viewer&apos;s thumb twitches toward the scroll gesture. @glowstate&apos;s current Reels average a <strong className="text-foreground">62% drop-off in the first 3 seconds</strong>. Study the first 3 seconds of your 10 favorite creators and pattern-match: what question are they making you ask? That&apos;s the hook.
                </p>

                <h4 className="mt-5 text-sm font-bold flex items-center gap-2">
                  <span className="grid size-5 place-items-center rounded-full bg-[color:var(--accent)] text-[0.6rem] font-bold text-white">20</span>
                  5 Mindset Shifts for More Leads
                </h4>
                <div className="mt-2 space-y-2.5">
                  {[
                    { num: "①", title: "Your content is a portfolio, not a diary.", body: "Every post should answer one question your ideal client is asking. If a post doesn&apos;t serve a stranger discovering you for the first time, it doesn&apos;t belong on your grid." },
                    { num: "②", title: "Consistency beats virality.", body: "One post per day for 90 days outperforms one viral post per month. The algorithm rewards predictable publishing, not sporadic spikes. Your current 4–5 posts/week cadence is close — tighten to daily." },
                    { num: "③", title: "Engagement is currency.", body: "Reply to every comment within 2 hours. Comment threads signal high-value content to the algorithm. A post with 20 comments and 15 replies ranks higher than one with 40 comments and 0 replies." },
                    { num: "④", title: "Trust trumps likability.", body: "Your audience doesn&apos;t need to like you — they need to trust you. Trust is built through demonstrated expertise, consistent value delivery, and proof (testimonials, lab results, before/after data). Stop trying to be charming. Start being useful." },
                    { num: "⑤", title: "You are a publication, not a product page.", body: "Treat your Instagram like a magazine: editorial calendar, content pillars, visual identity system, and a clear promise to the reader. Every post should earn its place in someone&apos;s feed. If it reads like a catalog entry, cut it." },
                  ].map((m) => (
                    <div key={m.num} className="rounded-[var(--radius-sm)] border border-border bg-[#fcfcfb] p-3">
                      <p className="text-xs"><span className="font-bold text-foreground">{m.num} {m.title}</span> <span className="text-muted-foreground">{m.body}</span></p>
                    </div>
                  ))}
                </div>

                {/* 20. Powered by AuditLayerMedia */}
                <div className="mt-6 rounded-md bg-[#1c1917] px-4 py-3 text-center">
                  <span className="text-sm font-bold text-white tracking-tight">ALM</span>
                  <span className="ml-2 text-xs text-white/70">Powered by AuditLayerMedia</span>
                  <p className="mt-1 text-[0.6rem] text-white/50">@auditlayermedia</p>
          ...
          <div className="mt-8 text-center">
            <Link href="/login">
              <Button className="font-semibold h-11 px-6">Start Your Audit</Button>
            </Link>
          </div>
        </section>
