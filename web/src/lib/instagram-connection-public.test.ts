...
describe("authenticated Instagram connection projections", () => {
  ...
  it("requires every authenticated connection query to use an allowlisted projection", () => {
    const pages = [
      ["src/app/api/resources/connections/route.ts", "INSTAGRAM_CONNECTION_CARD_FIELDS"],
    ...
    for (const [relativePath, projectionName] of pages) {
      ...
      expect(source.includes(`.select(${projectionName}`) || source.includes(".select(`id,${" + projectionName + "}`)")).toBe(true);
  ...
  it("keeps reconnect-required rows visible to the dashboard", () => {
    const source = readFileSync(
      path.join(process.cwd(), "src/app/api/resources/connections/route.ts"),
      "utf8",
    );
